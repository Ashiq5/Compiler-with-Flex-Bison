...
if(su.LookUp($3)!=NULL) {fprintf(logou,"ERROR:%d%s declared multiple times\n\n",line_count,$3);eror++;}
else
{
...
if(su.LookUp($1)!=NULL) {fprintf(logou,"ERROR:%d%s declared multiple times\n\n",line_count,$1);eror++;}
else{
...
$$=su.LookUp($1);
if($$==NULL){fprintf(logou,"ERROR:%s not declared\n\n",$1);eror++;}
if($$!=NULL && $$->a_size!=-1) {fprintf(logou,"ERROR:Type mismatch\n\n");eror++;}
}
...
if($$==NULL){
	...
	if(n!=NULL)m=n->a_size;
        ...
        if($3>=m){fprintf(logou,"ERROR: Array index out of bound\n\n");eror++;}
	else {fprintf(logou,"ERROR:%s not declared\n\n",$1);eror++;}
}
}
...
if($1!=NULL){
        if($1->Type=="INT"){
		if((int)$3!=$3){fprintf(logou,"ERROR : Type mismatch\n\n");eror++;}
                $1->val=(int)$3;
        }
        else if($1->Type=="CHAR"){
		if((int)$3!=$3){fprintf(logou,"ERROR: Type mismatch\n\n");eror++;}
...
term :	unary_expression                                {fprintf(logou,"term : unary_expression\n\n");$$=$1;}
     |  term MULOP unary_expression                     {fprintf(logou,"term : term MULOP unary_expression\n\n");
						       	 	...
						       	 	if($1==(int)$1 && $3==(int)$3)$$=(int)$1%(int)$3;
                                                         	else {fprintf(logou,"Error:Should be integer\n\n");eror++;}
                                                         }
                                                         printf("mulop %lf\n",$$);
							}
     ;
